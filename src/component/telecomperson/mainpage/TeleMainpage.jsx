...
const TeleMainPage = () => {
  ...
  const handleAdd =()=>{
    ...
    const AddnewItem={id,name,phoneno,gender,comment,status:'true',correction:""}
  ...
  const handleEdit =(id)=>{
    const editedData = datas.map((item)=>item.id === Number(id) ? {...item,status:'false',correction:mistake} :item)
    setDatas(editedData)
              ...
              {close => (
                <div className="popup">
                  ...
                  <div className="actions">
                    <button className="Admin-header-button-submit" onClick={() => { close(); toast("Insert Successfull");handleAdd() }}>submit</button>
                    ...
                    >
                      {close => (
                        <div className="popup">
                          <h3>Edit Data</h3>
                          <div className='pop-form'>
                            <input type='text' placeholder='Mistake' onChange={(e)=>setMistake(e.target.value)} value={mistake} required/>
